...
public class FileTest03 {
	...
	// 모든 파일과 디렉토리 목록을 출력하는 메소드
	public void displayFileList(File dir) {
		// 인수값으로 넘어온 값이 디렉토리 정보인지 확인
		if(!dir.isDirectory()) { // 디렉토리가 아니라면!
			System.out.println("디렉토리(폴더)만 가능합니다.");
			return; // 메소드를 종료시키기 위함 
		}
		
		System.out.println("[" + dir.getAbsolutePath() + "]");
		
		// 해당 디렉토리 안에 있는 모든 파일과 디렉토리 목록을 궇한다.
		File[] files = dir.listFiles(); // listFiles() => 디렉토리의 파일목록을 리스트로 반환한다.
