...
public class LprodIbatisTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//			환경 설정을 환성한 후 SQL문을 호출해서 실행함. 객체를 생성한다.
...
//			
			...
			// SELECT의 결과가 여러개일 경우에는 queryForList()메소드를 사용하는데
			// 이 메소드는 여러개의 레코드 각각을 VO에 담은 후 이 VO데이터들을
			// List에 추가해 주는 작업을 자동으로 수행한다.
			// 형식)	sqlMapClient.queryForList("namespace속성값.id속성값", 파라미터클래스)
//			List<LprodVO> lprodList =
//					smc.queryForList("lprod.getAllprod");
//			
//			for(LprodVO lvo3 : lprodList) {
//				System.err.println("ID : " + lvo3.getLprod_id());
...
//			System.out.println("---------------------------");
			...
			String lg3 = scan.next();
			
			// select한 결과가 1개가 확실할 경우에는 queryForObject()메소드를 사용한다.
			// 형식)	sqlMapClient객체변수.equeryForObject("namespace속성값.id속성값, 파라미터클래스)
