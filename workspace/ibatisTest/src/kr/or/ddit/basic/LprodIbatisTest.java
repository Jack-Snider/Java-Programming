...
public class LprodIbatisTest {
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         //1-3. 위에서 생성한 스트림 객체를 이용하여 환경 설정 파일을 읽어와 실행시켜 
      ...
      /*   System.out.println("insert 작업 시작...");
         ...
         //delete 실행
         //형식 sqlMapClient 객체변수.delete("namespace속성값.id속성값", 파라미터클래스)
         //   반환값 : 작업에 성공한 레코드 수
         ...
         */
         ...
         /*System.out.println("select 시작(결과가 여러개일경우..)");
         
         //select의 결과가 여러개일 경우에는 queryForList()메서드를 사용하는데
         //이 메서드는 여러개의 레코드 각각을 vo에 담은후 이vo데이터들을
         //list에 추가해 주는 작업을 자동으로 수행한다.
         //형식 ) sqlMapClient.queryForList("namespace 속성값.id속성값", 파라미터클래스)
         ...
         System.out.println("select 시작(결과가 한개일경우..)");
         ...
         //반환값 : 처리결과가 1개일 경우(정상) -> 해당 객체 반환
         // 처리 결과가 여러개 일경우 -> Exception객체 변환
