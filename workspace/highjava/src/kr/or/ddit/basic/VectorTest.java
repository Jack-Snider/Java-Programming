...
//		v1.remove('a');
		...
		/*
		 * 제네릭 타입(generic Type) ==> 클래스 내부에서 사용할 데이터의 데이터 타입을
		 * 외부에서 지정하는 기법으로 객체를 선언할 때
...
//		v2.add(100); 오류 --> 다른 종류의 데이터를 저장할 수 없다.
		...
		v2.removeAll(v4); //v2에 있는 데이터 중 v4가 가지고 있는 데이터와 일치하는 것들을 모두 삭제
		System.out.println("v2 지우고 남은 데이터 => " + v2);
